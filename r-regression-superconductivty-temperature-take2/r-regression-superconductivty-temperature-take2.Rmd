---
title: "Regression Model for Superconductivity Temperature Using R Take 2"
author: "David Lowe"
date: "July 17, 2019"
output: 
  html_document: 
...
In this iteration, we will examine the feature selection technique of attribute importance ranking by using the Elasticnet algorithm. By selecting only the most important attributes, we hope to decrease the modeling time and still maintain a similar level of RMSE compared to the baseline.
...
set.seed(seedNum)
model_fs <- train(targetVar~., data=xy_train, method="enet", preProcess="scale", trControl=control)
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(mtry = c(2,19,36,53,71))
...
# finalModel <- randomForest(targetVar~., xy_complete, mtry=19)
# summary(finalModel)
proc.time()-startTimeModule
```
