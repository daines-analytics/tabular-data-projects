---
title: "Binary Classification Model for Heart Disease Study Using R Take 2"
author: "David Lowe"
date: "May 28, 2019"
output: 
  html_document: 
...
In this iteration, we will examine the Hungarian dataset and create a machine learning model to fit the data.

ANALYSIS: The baseline performance of the machine learning algorithms achieved an average accuracy of 79.33%. Two algorithms (Logistic Regression and Gradient Boosting) achieved the top accuracy metrics after the first round of modeling. After a series of tuning trials, Gradient Boosting turned in the top overall result and achieved an accuracy metric of 82.13%. By using the optimized parameters, the Gradient Boosting algorithm processed the testing dataset with an accuracy of 82.76%, which was even better than the prediction accuracy gained from the training data.

CONCLUSION: For the Hungarian dataset, the Gradient Boosting algorithm achieved the best overall results using the training and testing datasets. For this dataset, Gradient Boosting could be considered for further modeling or production use.
...
xy_original$chol <- with(xy_original, impute(chol, median))
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(nrounds=c(10,25,50,100), max_depth=1, eta=0.3, gamma=0, colsample_bytree=0.8, min_child_weight=1, subsample=0.5)
