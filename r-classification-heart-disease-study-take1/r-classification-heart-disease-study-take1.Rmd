---
title: "Binary Classification Model for Heart Disease Study Using R Take 1"
author: "David Lowe"
date: "May 27, 2019"
output: 
  html_document: 
...
In this iteration, we will examine the Cleveland dataset and create a machine learning model to fit the data.

ANALYSIS: The baseline performance of the machine learning algorithms achieved an average accuracy of 84.78%. Two algorithms (Random Forest and Gradient Boosting) achieved the top accuracy metrics after the first round of modeling. After a series of tuning trials, Gradient Boosting turned in the top overall result and achieved an accuracy metric of 87.95%. By using the optimized parameters, the Gradient Boosting algorithm processed the testing dataset with an accuracy of 82.22%, which was slightly below the prediction accuracy gained from the training data.

CONCLUSION: For the Cleveland dataset, the Gradient Boosting algorithm achieved the best overall results using the training and testing datasets. For this dataset, Gradient Boosting could be considered for further modeling or production use.
...
xy_original$age <- as.integer(xy_original$age)
xy_original$sex <- as.integer(xy_original$sex)
xy_original$sex <- as.factor(xy_original$sex)
xy_original$cp <- as.integer(xy_original$cp)
xy_original$cp <- as.factor(xy_original$cp)
xy_original$trestbps <- as.integer(xy_original$trestbps)
xy_original$chol <- as.integer(xy_original$chol)
xy_original$fbs <- as.integer(xy_original$fbs)
xy_original$fbs <- as.factor(xy_original$fbs)
xy_original$restecg <- as.integer(xy_original$restecg)
xy_original$restecg <- as.factor(xy_original$restecg)
xy_original$thalach <- as.integer(xy_original$thalach)
xy_original$exang <- as.integer(xy_original$exang)
xy_original$exang <- as.factor(xy_original$exang)
xy_original$slope <- as.integer(xy_original$slope)
xy_original$slope <- as.factor(xy_original$slope)

# impute the missing values with values appropriate for the attributes
xy_original$ca <- with(xy_original, impute(ca, 0))
xy_original$ca <- as.integer(xy_original$ca)

xy_original$thal <- with(xy_original, impute(thal, 3))
xy_original$thal <- as.integer(xy_original$thal)
xy_original$thal <- as.factor(xy_original$thal)
```
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(mtry=c(2,6,9,13))
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(nrounds=c(25,50,100,150), max_depth=1, eta=0.3, gamma=0, colsample_bytree=0.8, min_child_weight=1, subsample=1)
...
# finalModel <- xgboost(data=x_final, label=y_final, nrounds=25, max_depth=1, eta=0.3, gamma=0, colsample_bytree=0.8, min_child_weight=1, subsample=1)
# print(finalModel)
proc.time()-startTimeModule
```
