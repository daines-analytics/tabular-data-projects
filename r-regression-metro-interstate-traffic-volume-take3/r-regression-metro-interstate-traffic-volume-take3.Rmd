...
In the current iteration, the baseline performance of the machine learning algorithms achieved an average RMSE of 814. Two algorithms (Random Forests and Gradient Boosting) achieved the top RMSE metrics after the first round of modeling. After a series of tuning trials, Random Forests turned in the top overall result and achieved an RMSE metric of 474. After applying the optimized parameters, the Random Forests algorithm processed the testing dataset with an RMSE of 472, which was slightly better than the prediction from the training data.
