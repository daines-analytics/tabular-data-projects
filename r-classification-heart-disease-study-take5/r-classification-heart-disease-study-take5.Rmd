---
title: "Binary Classification Model for Heart Disease Study Using R Take 5"
author: "David Lowe"
date: "May 31, 2019"
output: 
  html_document: 
...
In iteration Take3, we examined the Switzerland dataset and created a machine learning model to fit the data.

In iteration Take3, we examined the Long Beach VA dataset and created a machine learning model to fit the data.
...
xy_cleveland <- read.csv(inputFile, sep = ',', header = FALSE, col.names = colNames, na.strings = "?")
```
...
inputFile <- dest_file
colNames <- c('age','sex','cp','trestbps','chol','fbs','restecg','thalach','exang','oldpeak','slope','ca','thal','num')
xy_hungarian <- read.csv(inputFile, sep = ',', header = FALSE, col.names = colNames, na.strings = "?")
```
...
inputFile <- dest_file
colNames <- c('age','sex','cp','trestbps','chol','fbs','restecg','thalach','exang','oldpeak','slope','ca','thal','num')
xy_switzerland <- read.csv(inputFile, sep = ',', header = FALSE, col.names = colNames, na.strings = "?")
```
...
inputFile <- dest_file
colNames <- c('age','sex','cp','trestbps','chol','fbs','restecg','thalach','exang','oldpeak','slope','ca','thal','num')
xy_longbeach <- read.csv(inputFile, sep = ',', header = FALSE, col.names = colNames, na.strings = "?")
```

```{r}
xy_original <- rbind(xy_cleveland, xy_hungarian, xy_switzerland, xy_longbeach)
```
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(nrounds=c(50,100,150,200), max_depth=1, eta=0.3, gamma=0, colsample_bytree=0.8, min_child_weight=1, subsample=0.75)
...
# finalModel <- xgboost(data=x_final, label=y_final, nrounds=50, max_depth=1, eta=0.3, gamma=0, colsample_bytree=0.8, min_child_weight=1, subsample=0.5)
# print(finalModel)
proc.time()-startTimeModule
```
