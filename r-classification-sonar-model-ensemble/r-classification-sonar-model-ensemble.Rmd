---
title: "Ensemble Classification Model for the Sonar Dataset with R"
author: "David Lowe"
date: "March 16, 2018"
...
### 5.b) Ensembles
In this section, we will explore the use and tuning of RandomForest ensemble algorithm to see whether we can improve the results.
...
rf_default <- train(Class~., data=dataset, method="rf", metric=metricTarget, tuneGrid=tunegrid, trControl=control)
print(rf_default)
...
set.seed(seed_num)
mtry <- sqrt(ncol(x))
rf_random <- train(Class~., data=dataset, method="rf", metric=metricTarget, tuneLength=15, trControl=control)
print(rf_random)
plot(rf_random)
...
rf_gridsearch <- train(Class~., data=dataset, method="rf", metric=metricTarget, tuneGrid=tunegrid, trControl=control)
print(rf_gridsearch)
plot(rf_gridsearch)
...
for (ntree in c(1000, 1500, 2000, 2500)) {
	set.seed(seed_num)
	fit <- train(Class~., data=dataset, method="rf", metric=metricTarget, tuneGrid=tunegrid, trControl=control, ntree=ntree)
	key <- toString(ntree)
	modellist[[key]] <- fit
}
# compare results
...
set.seed(seed_num)
custom <- train(Class~., data=dataset, method=customRF, metric=metricTarget, tuneGrid=tunegrid, trControl=control)
print(custom)
