---
title: "Binary Classification Model for Credit Card Default Using R Take 1"
author: "David Lowe"
date: "July 9, 2018"
output: 
  html_document: 
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(sigma = c(.01, .05, 0.1), C = c(0.25, 0.5, 0.75, 1))
...
```{r POST_TUNING}
results <- resamples(list(ADA=fit.final1, SVM=fit.final2, GBM=fit.final3))
summary(results)
dotplot(results)
```
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(.n.trees=100, .shrinkage=0.1, .interaction.depth=3, .n.minobsinnode=3)
