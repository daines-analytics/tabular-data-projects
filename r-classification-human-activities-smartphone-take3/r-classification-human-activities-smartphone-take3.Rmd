---
title: "Multi-Class Classification Model for Human Activity Recognition with Smartphone Using R Take 3"
author: "David Lowe"
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(.n.trees=c(100,300,500,700,900), .shrinkage=c(0.1), .interaction.depth=c(1), .n.minobsinnode=c(10))
...
library(gbm)
set.seed(seedNum)
finalModel <- gbm(targetVar~., data=xy_train, n.trees=900, verbose=F)
summary(finalModel)
proc.time()-startTimeModule
```
