---
title: "Regression Model for Ames Iowa Housing Prices Using R Take 3"
author: "David Lowe"
date: "July 5, 2019"
output: 
  html_document: 
...
In this iteration, we will examine the feature selection technique of attribute importance ranking by using the Lasso algorithm. By selecting only the most important attributes, we hope to decrease the processing time and maintain a similar level of RMSE compared to the baseline.
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(mtry = c(2,12,23,33,44))
...
# finalModel <- gbm(targetVar~., xy_complete, n.trees = 150, interaction.depth = 3, shrinkage = 0.1, n.minobsinnode = 10, verbose = FALSE)
# summary(finalModel)
proc.time()-startTimeModule
```
