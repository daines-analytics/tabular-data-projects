---
title: "Regression Model for Metro Interstate Traffic Volume Using R Take 1"
author: "David Lowe"
date: "August 19, 2019"
output: 
  html_document: 
...
In this iteration, we will establish the baseline mean squared error for comparison with future rounds of modeling. This round of modeling will not include the date-time attribute.
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(nrounds=c(100,125,150,175,200), max_depth=3, eta=0.4, gamma=0, colsample_bytree=0.8, min_child_weight=1, subsample=0.75)
...
# finalModel <- randomForest(targetVar~., Xy_complete, mtry=6, na.action=na.omit)
# summary(finalModel)
proc.time()-startTimeModule
```
