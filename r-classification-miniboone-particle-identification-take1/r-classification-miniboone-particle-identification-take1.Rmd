---
title: "Binary Classification Model for MiniBooNE Particle Identification Using R Take 1"
author: "David Lowe"
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(mtry=c(5,15,25,40,50))
...
```{r POST_TUNING}
results <- resamples(list(BagCART=fit.final1, RF=fit.final2))
summary(results)
dotplot(results)
```
...
finalModel <- randomForest(targetVar ~ ., data = xy_complete, mtry=25)
print(finalModel)
proc.time()-startTimeModule
```
