---
title: "Binary Classification Model for Truck APS Failure Detection Using R Take 2"
author: "David Lowe"
...
CONCLUSION: The feature selection technique helped by cutting down the attribites and reduced the training time. Furthermore, the modeling took a much shorter time to process yet still retained an almost-identical level of accuracy. For this dataset, the Random Forest algorithm and the technique of eliminating collinear attributes should be considered for further modeling or production use.
...
```{r}
# Reconstruct the x_train dataframe after data cleaning
if (targetCol==1) {
x_train <- xy_train[,(targetCol+1):totCol]
} else {
x_train <- xy_train[,1:(totAttr)]
...
correlations <- cor(x_train)
highlyCorrelated <- findCorrelation(correlations, cutoff=0.75)
print(highlyCorrelated)
cat("Number of attributes found to be highly correlated:", length(highlyCorrelated))
```

```{r}
# Because the target variable is in the first column, we need to offset the highly correlated columns by 1
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(mtry = c(40,50,60))
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(.n.trees=c(800,900,1000,1100,1200), .shrinkage=0.1, .interaction.depth=c(1,2,3), .n.minobsinnode=10)
