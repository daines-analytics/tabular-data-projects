---
title: "Binary Classification Model for [Project Name] Using R version 04.0"
author: "David Lowe"
...
ANALYSIS: [Sample Paragraph - The baseline performance of the eight algorithms achieved an average accuracy of 76.07%. Three algorithms (Logistic Regression, Support Vector Machine, and Stochastic Gradient Boosting) achieved the top three accuracy scores after the first round of modeling. After a series of tuning trials, Support Vector Machine turned in the top result using the training data. It achieved an average accuracy of 77.64%. Using the optimized tuning parameter available, the Support Vector Machine algorithm processed the validation dataset with an accuracy of 76.96%, which was slightly below the accuracy of the training data.]

CONCLUSION: [Sample Paragraph - For this iteration, the Support Vector Machine algorithm achieved the top-tier training and validation results. For the project, Support Vector Machine should be considered for further modeling or production use.]
...
library(e1071)
set.seed(seedNum)
finalModel <- svm(targetVar ~ ., data = entireDataset, kernel = "radial", cost = 1, gamma = 0.015)
print(finalModel)
proc.time()-startTimeModule
```
