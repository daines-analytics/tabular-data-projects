---
title: "Binary Classification Model for [Project Name] Using R version 07"
author: "David Lowe"
date: "January 4, 2019"
output: 
  html_document: 
...
ANALYSIS: [Sample Paragraph - The baseline performance of the machine learning algorithms achieved an average accuracy of 74.84%. Two algorithms (Logistic Regression and Stochastic Gradient Boosting) achieved the top accuracy metrics after the first round of modeling. After a series of tuning trials, Stochastic Gradient Boosting turned in the top overall result and achieved an accuracy metric of 76.71%. By using the optimized parameters, the Stochastic Gradient Boosting algorithm processed the testing dataset with an accuracy of 76.09%, which was just slightly below the best prediction with the training data.]

CONCLUSION: [Sample Paragraph - For this iteration, the Stochastic Gradient Boosting algorithm achieved the top-tier training and validation results. For this dataset, the Stochastic Gradient Boosting algorithm should be considered for further modeling or production use.]
...
xy_original <- PimaIndiansDiabetes
...
# Logistic Regression has no special tuning parameters
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(.n.trees=c(50, 100, 200), .shrinkage=0.1, .interaction.depth=c(1, 2, 3), .n.minobsinnode=10)
...
```{r POST_TUNING}
results <- resamples(list(LR=fit.final1, GBM=fit.final2))
summary(results)
dotplot(results)
```
...
xy_complete$targetVar[xy_complete$oldTarget=="pos"] <- 1
xy_complete$oldTarget <- NULL

finalModel <- gbm(targetVar~., data=xy_complete, n.trees=100, shrinkage=0.1, interaction.depth=1, n.minobsinnode=10)
print(finalModel)
proc.time()-startTimeModule
```
