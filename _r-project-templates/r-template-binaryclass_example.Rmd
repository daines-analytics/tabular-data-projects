---
title: "Simple Binary Classification Model for [Project Name] Using R version 01.0"
author: "David Lowe"
date: "April 13, 2018"
output: 
  html_document: 
...
```{r}
#entireDataset <- read.csv("some_dataset.csv", header= TRUE)
...
```{r SPOT_CHECK}
results <- resamples(list(LogReg=fit.glm, LDA=fit.lda, CART=fit.cart, NB=fit.nb, kNN=fit.knn, SVM=fit.svm, BagCART=fit.bagcart, RF=fit.rf, AdaBoost=fit.ada, GBM=fit.gbm))
summary(results)
dotplot(results)
```
...
```{r}
set.seed(seedNum)
finalModel <- glm(targetVar~., data=entireDataset, family="binomial")
summary(finalModel)
```
