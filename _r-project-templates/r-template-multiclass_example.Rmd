---
title: "Simple Multi-Classification Model for [Project Name] Using R version 02.0"
author: "David Lowe"
date: "May 22, 2018"
output: 
  html_document: 
...
Dataset ML Model: Multi-classification with [numerical | categorical] attributes
...
For this project, we will evaluate one linear, four non-linear, and three ensemble algorithms:
...
```{r SPOT_CHECK}
results <- resamples(list(LDA=fit.lda, CART=fit.cart, NB=fit.nb, kNN=fit.knn, SVM=fit.svm, BagCART=fit.bagcart, RF=fit.rf, GBM=fit.gbm))
summary(results)
dotplot(results)
```
