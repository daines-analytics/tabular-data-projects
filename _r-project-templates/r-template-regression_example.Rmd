---
title: "Regression Model for [Project Name] Using R version 07"
author: "David Lowe"
date: "January 4, 2019"
output: 
  html_document: 
...
ANALYSIS: [Sample Paragraph - The baseline performance of the machine learning algorithms achieved an average RMSE of 4.492. Two algorithms (Random Forest and Stochastic Gradient Boosting) achieved the top RMSE metrics after the first round of modeling. After a series of tuning trials, Stochastic Gradient Boosting turned in the top overall result and achieved an RMSE metric of 3.083. By using the optimized parameters, the Stochastic Gradient Boosting algorithm processed the testing dataset with a RMSE of 3.554, which was slightly below the best prediction with the training data.]
...
xy_original[,'chas'] <- as.numeric(xy_original[,'chas'])
```
...
```{r SPOT_CHECK}
results <- resamples(list(LR=fit.lm, CART=fit.cart, kNN=fit.knn, BagCART=fit.bagcart, RF=fit.rf, GBM=fit.gbm))
summary(results)
dotplot(results)
cat('The average RMSE from all models is:',
    mean(c(results$values$`LR~RMSE`,results$values$`CART~RMSE`,results$values$`kNN~RMSE`,results$values$`BagCART~RMSE`,results$values$`RF~RMSE`,results$values$`GBM~RMSE`)))
```
...
finalModel <- gbm(targetVar~., data=xy_complete, n.trees=550, shrinkage=0.1, interaction.depth=3, n.minobsinnode=10)
summary(finalModel)
proc.time()-startTimeModule
```
