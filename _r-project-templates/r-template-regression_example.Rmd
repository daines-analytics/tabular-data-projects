---
title: "Regression Model for [Project Name] Using R version 04.0"
author: "David Lowe"
...
ANALYSIS: [Sample Paragraph - The baseline performance of the seven algorithms achieved an average RMSE of 4.4119. Three algorithms (Bagged CART, Random Forest, and Stochastic Gradient Boosting) achieved the top three RMSE scores after the first round of modeling. After a series of tuning trials, Stochastic Gradient Boosting turned in the top result using the training data. It achieved an average RMSE of 2.7923. Using the optimized tuning parameter available, the Stochastic Gradient Boosting algorithm processed the validation dataset with a RMSE of 3.5912, which was slightly worst than the RMSE of the training data.

CONCLUSION: [Sample Paragraph - For this iteration, the Stochastic Gradient Boosting algorithm achieved the top-tier training and validation results. For the project, Stochastic Gradient Boosting should be considered for further modeling or production use.]
...
library(gbm)
set.seed(seedNum)
#grid <- expand.grid(.n.trees=400, .interaction.depth=3, .shrinkage=0.1, .n.minobsinnode=1)
#finalModel <- train(targetVar~., data=entireDataset, method="gbm", metric=metricTarget, tuneGrid=grid, trControl=control, verbose=F)
finalModel <- gbm(targetVar~., data=entireDataset, n.trees=400, shrinkage=0.1, interaction.depth=3, n.minobsinnode=1)
summary(finalModel)
proc.time()-startTimeModule
```
