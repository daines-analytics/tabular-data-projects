---
title: "Regression Model for [Project Name] Using R version 05.0"
author: "David Lowe"
...
ANALYSIS: [Sample Paragraph - The baseline performance of the seven algorithms achieved an average RMSE of 4.4119. Three algorithms (Bagged CART, Random Forest, and Stochastic Gradient Boosting) achieved the top three RMSE scores after the first round of modeling. After a series of tuning trials, Stochastic Gradient Boosting turned in the top result using the training data. It achieved an average RMSE of 3.0830. Using the optimized tuning parameter available, the Stochastic Gradient Boosting algorithm processed the validation dataset with a RMSE of 3.5549, which was slightly below the training data and possibly due to over-fitting.]

CONCLUSION: [Sample Paragraph - For this iteration, the Stochastic Gradient Boosting algorithm achieved the top training and validation results. For this dataset, the Stochastic Gradient Boosting algorithm should be considered for further modeling or production use.]
...
originalDataset[,'chas'] <- as.numeric(originalDataset[,'chas'])
```
...
cat('The average RMSE from all models is:',
    mean(c(results$values$`LR~RMSE`,results$values$`CART~RMSE`,results$values$`kNN~RMSE`,results$values$`SVM~RMSE`,results$values$`BagCART~RMSE`,results$values$`RF~RMSE`,results$values$`GBM~RMSE`)))
```
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(.n.trees=c(250, 400, 550), .shrinkage=0.1, .interaction.depth=c(1, 2, 3), .n.minobsinnode=10)
...
library(gbm)
set.seed(seedNum)
finalModel <- gbm(targetVar~., data=xy_train, n.trees=550, shrinkage=0.1, interaction.depth=3, n.minobsinnode=10)
summary(finalModel)
proc.time()-startTimeModule
```
