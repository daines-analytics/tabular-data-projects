---
title: "Binary Classification Model for Truck APS Failure Detection Using R Take 3"
author: "David Lowe"
...
```{r}
# Using the bagged CART (treebag) algorithm, we perform the Recursive Feature Elimination (RFE) technique
startTimeModule <- proc.time()
set.seed(seedNum)
maxSubset <- 50
rfeCTRL <- rfeControl(functions=treebagFuncs, method="cv", number=10)
rfeResults <- rfe(xy_train[,1:totAttr], xy_train[,totCol], sizes=c(1:maxSubset), rfeControl=rfeCTRL)
print(rfeResults)
...
# Reattach the targetVar column name to the dataframes
targetCol <- ncol(xy_train)
colnames(xy_train)[targetCol] <- "targetVar"
colnames(xy_test)[targetCol] <- "targetVar"
```
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(.n.trees=c(500,650,800,950,1100), .shrinkage=0.1, .interaction.depth=3, .n.minobsinnode=10)
...
set.seed(seedNum)
finalModel <- randomForest(targetVar~., data=xy_train, mtry=10)
summary(finalModel)
proc.time()-startTimeModule
```
