---
title: "Binary Classification Model for Diabetes Readmission Prediction Using R Take 3"
author: "David Lowe"
date: "May 17, 2019"
output: 
  html_document: 
...
In iteration Take2, we further tested the machine learning models by rearranging some of the features to be more consistent with the research paper (Table 4). We had hoped to improve the overall accuracy and applicability of the model by having features with a fewer number of categories.

In this iteration, we will test the machine learning models by reconfiguring the target variable to have only two categories, thus making this a binary classification exercise. We hope to improve the overall accuracy applicability of the model by predicting with just the "yes" and "no" outcomes.

ANALYSIS: The baseline performance of the machine learning algorithms achieved an average accuracy of 61.82%. Two algorithms (Logistic Regression and Gradient Boosting) achieved the top accuracy metrics after the first round of modeling. After a series of tuning trials, Gradient Boosting turned in the top overall result and achieved an accuracy metric of 63.05%. By using the optimized parameters, the Gradient Boosting algorithm processed the testing dataset with an accuracy of 62.96%, which was slightly below the prediction accuracy using the training data.

CONCLUSION: Restructuring the target variable to binary outcomes yielded accuracy improvement and a reduction in the processing time. For this iteration, the Gradient Boosting algorithm achieved the overall results using the training and testing datasets. For this dataset, Gradient Boosting should be considered for further modeling or production use.
...
```{r}
# Transform the race codes to their corresponding categories
reClassReadmit <- function(readmit_code)
{
readmit_category <-
  if ((readmit_code==">30") || (readmit_code=="<30")) 1 else
                                                      0
return(readmit_category)
}

xy_original$readmitted <- sapply(xy_original$readmitted, reClassReadmit)
xy_original$readmitted <- as.factor(xy_original$readmitted)
```
...
# Logistic Regression has no special tuning parameters, so we are skipping this section.
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(nrounds=c(100,200,300), max_depth=3, eta=0.3, gamma=0, colsample_bytree=0.6, min_child_weight=1, subsample=1)
...
```{r POST_TUNING}
results <- resamples(list(LR=fit.final1, XGB=fit.final2))
summary(results)
dotplot(results)
```
...
# finalModel <- xgboost(data=x_final, label=y_final, nrounds=300, max_depth=3, eta=0.3, gamma=0, colsample_bytree=0.6, min_child_weight=1, subsample=1)
# summary(finalModel)
proc.time()-startTimeModule
```
