---
title: "Regression Model for Ames Iowa Housing Prices Using R Take 1"
author: "David Lowe"
date: "July 1, 2019"
output: 
  html_document: 
...
SUMMARY: The purpose of this project is to construct a prediction model using various machine learning algorithms and to document the end-to-end steps using a template. The Ames Iowa Housing Prices dataset is a regression situation where we are trying to predict a real-number value for a given set of criteria.
...
xy_original$BsmtQual <- with(xy_original, impute(BsmtQual, 'NoBasement'))
xy_original$BsmtQual <- as.factor(xy_original$BsmtQual)
xy_original$BsmtCond <- with(xy_original, impute(BsmtCond, 'NoBasement'))
xy_original$BsmtCond <- as.factor(xy_original$BsmtCond)
xy_original$BsmtExposure <- with(xy_original, impute(BsmtExposure, 'NoBasement'))
xy_original$BsmtExposure <- as.factor(xy_original$BsmtExposure)
xy_original$BsmtFinType1 <- with(xy_original, impute(BsmtFinType1, 'NoBasement'))
xy_original$BsmtFinType1 <- as.factor(xy_original$BsmtFinType1)
xy_original$BsmtFinType2 <- with(xy_original, impute(BsmtFinType2, 'NoBasement'))
xy_original$BsmtFinType2 <- as.factor(xy_original$BsmtFinType2)
xy_original$Electrical <- with(xy_original, impute(Electrical, 'Unknown'))
xy_original$Electrical <- as.factor(xy_original$Electrical)
xy_original$FireplaceQu <- with(xy_original, impute(FireplaceQu, 'NoFireplace'))
xy_original$FireplaceQu <- as.factor(xy_original$FireplaceQu)
...
xy_original$GarageFinish <- with(xy_original, impute(GarageFinish, 'NoGarage'))
xy_original$GarageFinish <- as.factor(xy_original$GarageFinish)
xy_original$GarageQual <- with(xy_original, impute(GarageQual, 'NoGarage'))
xy_original$GarageQual <- as.factor(xy_original$GarageQual)
xy_original$GarageCond <- with(xy_original, impute(GarageCond, 'NoGarage'))
xy_original$GarageCond <- as.factor(xy_original$GarageCond)
xy_original$PoolQC <- with(xy_original, impute(PoolQC, 'NoPool'))
xy_original$PoolQC <- as.factor(xy_original$PoolQC)
...
xy_original$ExterQual <- as.factor(xy_original$ExterQual)
xy_original$ExterCond <- as.factor(xy_original$ExterCond)
xy_original$Foundation <- as.factor(xy_original$Foundation)
xy_original$Heating <- as.factor(xy_original$Heating)
xy_original$HeatingQC <- as.factor(xy_original$HeatingQC)
xy_original$CentralAir <- as.factor(xy_original$CentralAir)
xy_original$KitchenQual <- as.factor(xy_original$KitchenQual)
...
xy_original$ExterCond <- NULL
xy_original$BsmtCond <- NULL
xy_original$Heating <- NULL
xy_original$HeatingQC <- NULL
xy_original$Electrical <- NULL
xy_original$Functional <- NULL
xy_original$PoolQC <- NULL
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(fraction = c(0.05, 0.25, 0.525, 0.75, 1.0), lambda = 0.0001)
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(n.trees = c(50,150,250,350,500), interaction.depth = 3, shrinkage = 0.1, n.minobsinnode = 10)
...
```{r POST_TUNING}
results <- resamples(list(ENET=fit.final1, GBM=fit.final2))
summary(results)
dotplot(results)
```
...
# finalModel <- gbm(targetVar~., xy_complete, n.trees = 350, interaction.depth = 3, shrinkage = 0.1, n.minobsinnode = 10, verbose = FALSE)
# summary(finalModel)
proc.time()-startTimeModule
```
