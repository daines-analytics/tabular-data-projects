---
title: "Binary Classification Model for MiniBooNE Particle Identification Using R Take 2"
author: "David Lowe"
date: "January 7, 2019"
output: 
  html_document: 
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(.n.trees=c(250,500,750), .shrinkage=0.1, .interaction.depth=c(1,2,3), .n.minobsinnode=10)
...
startTimeModule <- proc.time()
set.seed(seedNum)
grid <- expand.grid(mtry=c(1,5,15,25,35))
...
finalModel <- gbm(targetVar ~ ., data = xy_complete, n.trees = 750, interaction.depth = 3, shrinkage = 0.1, n.minobsinnode = 10)
print(finalModel)
proc.time()-startTimeModule
```
