---
title: "Simple Classification of Titanic Dataset, Take 3"
author: "David Lowe"
...
```{r}
setwd("C:/Users/David/DEV/GitHub/r-classification-Titanic-ensemble")
...
For the take #3 version of the project, we will add the sixth and seventh iterations of experimenting with several machine learning algorithms. We will see whether the machine learning algorithms can improve our predictions.
...
6. Leverage ensembles of algorithm to generate predictions
7. Tune the best-performing ensemble by experimenting various parameters
...
set.seed(seed_num)
fit.gbm <- train(Survived~., data=training, method="gbm", metric=metric, trControl=control, verbose=FALSE, na.action=na.omit)
# Bagged CART
set.seed(seed_num)
fit.treebag <- train(Survived~., data=training, method="treebag", metric=metric, trControl=control, na.action=na.omit)
# Random Forest
set.seed(seed_num)
fit.rf <- train(Survived~., data=training, method="rf", metric=metric, trControl=control, na.action=na.omit)
# Conditional Inference Random Forest
set.seed(seed_num)
fit.cforest <- train(Survived~., data=training, method="cforest", metric=metric, trControl=control, na.action=na.omit)
# eXtreme Gradient Boosting
set.seed(seed_num)
fit.xgb <- train(Survived~., data=training, method="xgbTree", metric=metric, trControl=control, na.action=na.omit)
# Compare results
ensemble_results <- resamples(list(GBM=fit.gbm, BAG=fit.treebag, RF=fit.rf, CF=fit.cforest, XGB=fit.xgb))
...
summary(predictions)
submission <- data.frame(PassengerId=validation$PassengerId, Survived=predictions)
write.csv(submission, file="submit-iteration6.csv", row.names=FALSE)
```

From iteration #6, we achieved an accuracy rate of 0.75598, which was worst than the result from iteration #5.
...
fit.rf <- train(Survived~., data=training, method="rf", metric=metric, tuneGrid=grid, trControl=control, na.action=na.omit)
print(fit.rf)
plot(fit.rf)
predictions <- predict(fit.rf, newdata=validation)
summary(predictions)
submission <- data.frame(PassengerId=validation$PassengerId, Survived=predictions)
write.csv(submission, file="submit-iteration7.csv", row.names=FALSE)
```
...
Although ensemble turned out not to be the best model, we will document the modeling statements here for learning purposes.

```{r}
library(randomForest)
set.seed(seed_num)
finalModel <- randomForest(Survived~., training, mtry=4, na.action=na.omit)
summary(finalModel)
```
